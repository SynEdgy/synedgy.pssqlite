...
- If this repository adds custom GitHub wiki pages, keep the hand-authored pages under `source\WikiSource`.
...
- Ensure docs or build workflows that prepare wiki content include `Copy_Source_Wiki_Folder` so `source\WikiSource` is copied into `output\WikiContent`.
- Keep `Publish_GitHub_Wiki_Content` in the `publish` workflow when this repository publishes generated wiki content.
...
- If `Home.md` contains module version placeholders, rely on the Sampler and DscResource.DocGenerator tasks to update them during the build; do not hard-code release-specific versions into committed wiki pages.
...
- When changing publish behavior, also validate the docs or publish path used by the pipeline rather than editing `output\WikiContent` manually.
